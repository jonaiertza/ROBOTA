...
int k = 20;                   //Hasierako posizioa ukondoa
int j = 90;                   //Hasierako posizioa eskua
int zilindroa = 11;
int angeluaSerbo2 = 30;
int gehikuntza = 5;
int gehikuntzak = 5;
...
void loop() {                                         
  ...
  Serial.println(" ");
  Serial.println(" ");
  //ukondoa                                         k 0 eta 180 graduren bitartean joystickarekin egiten diogun gehikuntzaren arabera
  if  (balioa_1 == LOW) {
    k = k + gehikuntzak;
    k = constrain(k, 0, 180);             
    setServo(13, k);
  ...
  //pintza botoi horiak                            j 0 eta 170 graduren bitartean botoi horiekin egiten diogun gehikuntzaren arabera
  if (balioa_3 == LOW) {
    j = j + gehikuntzaj;
    j = constrain(j, 5, 170);
    setServo(14, j);
  ...
  // zilindro pneumatikoa                               GUK EZ DUGU HAU ERABILI!!!!
  ...
  switch (i) {
    case 1:
      //-------- 1 SERBOA -- (OINARRIA !!!!) ---------------//----------                ABIADURAZKO SERBOAREN PARAMETROAK
      if ((x >= 0) && (x <= 454)) {
        setServo(1, 70);                        //ABIADURA EZKERRETARA
        ...
        setServo(1, 98);                       //ABIADURA ESKUBITARA
        Serial.print("Oinarria: ");
        Serial.println("ABIADURA +");
      }
      //---------- 2 SERBOA ---------------------------
